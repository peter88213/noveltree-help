...
Sets the `type <basic_concepts.html#part-chapter-section-types>`__ of the selected chapter or section.
...
Sets the `completion status <basic_concepts.html#section-completion-status>`__ of the selected section.
...
-  Select the *Characters* root node to set the status for all
   characters.
